...
const auto base {
    BoardRepr::FromString("1;0;0;B;R;2;0;1;B;H;3;0;2;B;B;4;0;3;B;Q;5;0;4;B;K;6;0;5;B;B;7;0;6;B;H;8;0;7;B;R;"
...
const auto baseReversed {
    BoardRepr::FromString("1;0;0;W;R;2;0;1;W;H;3;0;2;W;B;4;0;3;W;Q;5;0;4;W;K;6;0;5;W;B;7;0;6;W;H;8;0;7;W;R;"
...
bool is_legal_move(Input&& move, FigureType ft)
{
    ...
    return std::find_if(moves.begin(), moves.end(), [&](auto p) { return p.second == move.target; }) != moves.end();
}
...
TEST_CLASS(PawnMoves) { public: TEST_METHOD(Basic
) { for (auto [c, idw]
...
{
    ChessGame board { BoardRepr { { new Figure(0_id, Pos { 3, 2 }, Color::White, FigureType::Pawn),
    ...
    {
        auto [conducted, msg] = board.provide_move(
            black_pawn, Input { black_pawn->get_pos(), Pos { 3, 3 } }, Color::Black, standard_provider
        );
    ...
    Assert::IsTrue(std::find_if(moves.begin(), moves.end(), [](auto p) {
                       return p.second == Pos { 2, 3 };
                   }) != moves.end());
...

    {
        auto [conducted, mm] = board.provide_move(
            white_pawn, Input { white_pawn->get_pos(), Pos { 2, 3 } }, Color::White, standard_provider
        );
        Assert::IsTrue(conducted);
...
{
    ChessGame board { BoardRepr { { new Figure(0_id, Pos { 3, 2 }, Color::Black, FigureType::Pawn),
    ...
    {
        auto [conducted, msg] = board.provide_move(
            white_pawn, Input { white_pawn->get_pos(), Pos { 3, 3 } }, Color::Black, standard_provider
        );
        Assert::IsTrue(conducted);
        board.set_last_move(msg);
    }
    ...
    {
        auto [conducted, mm] = board.provide_move(
            black_pawn, Input { black_pawn->get_pos(), Pos { 2, 3 } }, Color::White, standard_provider
        );
        Assert::IsTrue(conducted);
...
TEST_METHOD(Promotion)
{
    ...
    Assert::IsTrue(ms.to_eat.empty() && ms.to_move.empty() && ms.what_castling_breaks.empty());

    {
        auto [conducted, msg] = board.provide_move(
            white_pawn, Input { white_pawn->get_pos(), Pos { 0, 5 } }, Color::White, standard_provider
        );
        Assert::IsTrue(conducted);
...
TEST_METHOD(PromotionReversed)
{
    ...
    {
        auto [conducted, msg] = board.provide_move(
            black_pawn, Input { black_pawn->get_pos(), Pos { 0, 5 } }, Color::White, standard_provider
        );
        Assert::IsTrue(conducted);
        Assert::AreEqual(msg.ms.main_ev, MainEvent::MOVE);
        Assert::IsTrue(contains(msg.ms.side_evs, SideEvent::PROMOTION));
...
TEST_CLASS(KnightMoves) { public: TEST_METHOD(Basic) { const std::size_t moves[][8] { { 2, 3, 4, 4, 4, 4, 3, 2 },
                                                                                      { 3, 4, 6, 6, 6, 6, 4, 3 },
                                                                                      { 4, 6, 8, 8, 8, 8, 6, 4 },
                                                                                      { 4, 6, 8, 8, 8, 8, 6, 4 },
                                                                                      { 4, 6, 8, 8, 8, 8, 6, 4 },
...
TEST_CLASS(BishopMoves) { public: TEST_METHOD(Basic) { move_check_assert(
...
TEST_CLASS(RookMoves) { public: TEST_METHOD(Basic) { move_check_assert(
...
TEST_CLASS(QueenMoves) { TEST_METHOD(Basic) { move_check_assert(
...
TEST_CLASS(KingMoves) { public: TEST_METHOD(Basic) { move_check_assert(
