...
public:
    ...
    FN operator<=>(const Figure& r) const noexcept { return this->id_ <=> r.id_; }

    FN get_id() const noexcept -> Id { return id_; }

    FN get_pos() const noexcept -> Pos { return position_; }

    FN get_col() const noexcept -> Color { return color_; }

    FN get_type() const noexcept -> FigureType { return type_; }
