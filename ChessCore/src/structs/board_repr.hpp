...
struct BoardRepr
{
    ...
    [[nodiscard]] constexpr BoardRepr(BoardRepr&& br) noexcept
    {
        clear();
        *this = br;
    }
    ...
    constexpr BoardRepr& operator=(const BoardRepr& other) noexcept
    {
        this->clear();
        for (const auto& fig : other.figures) {
...
struct FromString<board_repr::BoardRepr>
{
        ...
        -> ParseEither<board_repr::BoardRepr, ParseErrorType>
    {
        switch (meta.version) {
            case 2:
                return parse_v2(sv, meta);
            default:
                return PARSE_STEP_UNEXPECTED(ParseErrorType, Meta_UnsupportedVersion, 0ull);
        }
    }
    ...
    operator()(const std::string_view sv) const noexcept -> ParseEither<board_repr::BoardRepr, ParseErrorType>
    {
        ...
        meta.turn = sv_meta[pos] == 'B' ? Color::Black : Color::White;
        pos += 1;
        if (sv_meta.size() != pos) {
            if (sv_meta[pos] != 'C') {
                return PARSE_STEP_UNEXPECTED(ParseErrorType, Meta_CouldNotFindCastlings, pos);
            }
            for (const auto& castling : split(sv_meta.substr(pos + 1, sv_meta.size() - pos - 1), ","sv)) {
...
struct AsString<board_repr::BoardRepr>
{
    ...
    FN operator()(const board_repr::BoardRepr& br) const noexcept -> std::string
    {
        ...
        meta.min_id = std::ranges::fold_left_first(
                          br.figures + br.captured_figures,
                          [](Figure const* acc, Figure const* fig) { return fig->get_id() < acc->get_id() ? fig : acc; }
        )
                          .transform(&Figure::get_id)
                          .value_or(0_id);
