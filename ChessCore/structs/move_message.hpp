        ...
        -> std::string
    {
        std::string result{
            USING_BEG(&)
                std::string result{
                    ...
                    + as_string<FigureType>{}(move_message.promotion_choice, meta)
                };
        ...
        result += VISIT(move_message.main_event,
            ...
            [&](const mvmsg::Castling& castling) {
                return "C"s
                    + as_string<Id>{}(castling.second_to_move, meta)
            ...
            [&](const mvmsg::EnPassant& en_passant) constexpr {
                return "P"s
                    + as_string<Id>{}(en_passant.eaten, meta);
            }
        );
