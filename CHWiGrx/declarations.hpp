...
} inline figures_list { FIGURES_LIST_WINDOW_DEFAULT_POS,
...
public:
    ...
    void init_curr_choice_window(const HWND h_wnd, const WNDPROC callback)
    {
        ...
        POINT mouse {};
        GetCursorPos(&mouse);
        curr_chose_window_ = create_curr_choice_window(
            h_wnd, in_hand_.value(), mouse, main_window.get_cell_width(), main_window.get_cell_height(), callback
        );
        RedrawWindow(
